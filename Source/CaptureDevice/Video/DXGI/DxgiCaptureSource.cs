...
namespace Aperture {
  ...
  internal class DxgiCaptureSource : IDisposable {
    ...
    /// </summary>
    private Output1 Output1 { get; }

    /// <summary>
    ///   DXGI 1.6 output associated with this source
    /// </summary>
    private Output6 Output6 { get; }
    ...
    /// </summary>
    internal OutputDuplication Duplication { get; }
      ...
      using (var factory = new Factory1()) {
        ...
        foreach (Adapter1 adapter in factory.Adapters1) {
          foreach (Output output in adapter.Outputs) {
            Rectangle intersection = Rectangle.Intersect(bounds, output.Description.DesktopBounds);

            if (intersection.Width > 0 && intersection.Height > 0) {
              source = new DxgiCaptureSource(adapter, output,
                                             new Rectangle(intersection.X, intersection.Y, intersection.Width,
                                                           intersection.Height));
              break;
            }
